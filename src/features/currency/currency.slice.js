...
export const selectCurrentAmount = (state) => parseFloat(state.currency.visibleCurrencies[state.currency.currentCurrency]);
...
// export thunk's
//
export const changeCurrency = (currencyCode) => (dispatch, getState) => {
  dispatch(setCurrentCurrency(currencyCode));
  
  const currentValue = selectVisibleCurrencies(getState())[currencyCode];
  dispatch(setKeypadValue(currentValue.toString().split('')));
}
